...
_CANSetPara Can2SetPataInfo = NULL;
_CANSetPara Can3SetPataInfo = NULL;
...
uint8_t RxData[64];
uint8_t TxData[64] = {0x10, 0x32, 0x54, 0x76, 0x98, 0x00, 0x11, 0x22,0x32, 0x54, 0x76, 0x98, 0x00, 0x11, 0x22,0x32, 0x54, 0x76, 0x98, 0x00, 0x11, 0x22,0x32, 0x54, 0x76, 0x98};
...
{	
  ...
  hfdcan2.Init.FrameFormat = FDCAN_FRAME_FD_NO_BRS;//FDCAN_FRAME_CLASSIC;
  ...
  ***/
  ...
  /* Start the FDCAN module */
   ...
   /*** 
  if (HAL_FDCAN_ConfigTxDelayCompensation(&hfdcan2, hfdcan2.Init.DataPrescaler * hfdcan2.Init.DataTimeSeg1, 0) != HAL_OK)
	{
	  Error_Handler();
	  MyPrintf("HAL_FDCAN_ConfigTxDelayCompensation error \n");
	}
	if (HAL_FDCAN_EnableTxDelayCompensation(&hfdcan2) != HAL_OK)
	{
	  Error_Handler();
	   MyPrintf("HAL_FDCAN_ConfigTxDelayCompensation error Too\n");
	}
   ****/
  if (HAL_FDCAN_Start(&hfdcan2) != HAL_OK)
  {
...
void CAN_BANDRate_Set(FDCAN_InitTypeDef *FDCANInit,uint16_t normalBandRate,uint32_t dataBandRate)
{
	...
	if((FDCANInit->FrameFormat == FDCAN_FRAME_CLASSIC) || (FDCANInit->FrameFormat == FDCAN_FRAME_FD_NO_BRS))
	{
		switch(normalBandRate)
		{
			case 50:
				nominalPrescaler = 160;						//×ÜÏßÆµÂÊ160M-²ÉÑùÂÊÎª80%
		...
		FDCANInit->NominalPrescaler = nominalPrescaler;
		FDCANInit->NominalSyncJumpWidth = nominalSyncJumpWidth;
		FDCANInit->NominalTimeSeg1 = nominalTimeSeg1;
		FDCANInit->NominalTimeSeg2 = nominalTimeSeg2;
		FDCANInit->DataPrescaler = nominalPrescaler;
		FDCANInit->DataSyncJumpWidth = nominalSyncJumpWidth;
		FDCANInit->DataTimeSeg1 = nominalTimeSeg1;
		FDCANInit->DataTimeSeg2 = nominalTimeSeg2;
	}
	else
	{
		switch(normalBandRate)
		...
		{
			case 100:
				dataPrescaler = 32;		//×ÜÏßÆµÂÊ160M-²ÉÑùÂÊÎª80%
				dataSyncJumpWidth = 1;
				dataTimeSeg1 = 39;
				dataTimeSeg2 = 10;
				break;
			...
			case 4000:
				...
				dataTimeSeg2 = 2;
				break;
			case 5000:							//×ÜÏßÆµÂÊ160M-²ÉÑùÂÊÎª75%
				dataPrescaler = 2;
				dataSyncJumpWidth = 1;
				dataTimeSeg1 = 8;
				dataTimeSeg2 = 7;
		...
		FDCANInit->DataPrescaler = dataPrescaler;
		FDCANInit->DataSyncJumpWidth = dataSyncJumpWidth;
		FDCANInit->DataTimeSeg1 = dataTimeSeg1;
		FDCANInit->DataTimeSeg2 = dataTimeSeg2;
  ...
  switch(AppStep)
  {
		...
		case 1:
				 ...
				 {  
					 ...
					 MyPrintf("five seconds here-%u\r\n",AppTime);
					 MyPrintf("HAL_RCC_GetSysClockFreq:%u\r\n",HAL_RCC_GetSysClockFreq());
					  MyPrintf("HAL_RCC_GetHCLKFreq:%u\r\n",HAL_RCC_GetHCLKFreq());
					   MyPrintf("HAL_RCC_GetPCLK1Freq:%u\r\n",HAL_RCC_GetPCLK1Freq());
					    MyPrintf("HAL_RCC_GetPCLK2Freq:%u\r\n",HAL_RCC_GetPCLK2Freq());
...
void CAN2_Data_Pack(FDCAN_RxHeaderTypeDef TRX,uint8_t *buf)
{
	...
	MyPrintf("iType:%08X,RFType:%08X,len:%02X\n",TRX.IdType,TRX.RxFrameType,TRX.DataLength);
	MyPrintf("-estateI:%08X,bRateS:%08X,fFormat:%08X\n",TRX.ErrorStateIndicator,TRX.BitRateSwitch,TRX.FDFormat);
